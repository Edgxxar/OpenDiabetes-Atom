...
export default {
  ...
  activate(state) {
    this.subscriptions = new CompositeDisposable(
      ...
      atom.commands.add('atom-workspace', {
        ...
        'open-diabetes-filter:editor-file': e => {
          // open blockly editor
          atom.workspace.open('atom://open-diabetes-filter').then(editor => {
            // wait for blockly to be fully injected
            editor.blockly_promise.then(() => {
              // try to load the file
              const file = e.target.dataset.path;
              if (file !== null)
                editor.loadFromFile(file);
            });
          })
        },
        'open-diabetes-filter:initialize-repository': () => util.initializeVault(),
  ...
  checkSettings(silent) {
    if (atom.config.get('open-diabetes-filter.docker')) {
      util.checkDocker(silent);
    } else {
      util.checkCli(silent);
      util.checkPlot(silent);
    }
  },
  ...
  loadBlockly() {
    ...
    Promise.all([
      ...
      // listen for .odf files
      this.subscriptions.add(atom.workspace.onDidAddTextEditor(item => {
        const file = item.textEditor.getPath();
        if (path.extname(file) === '.odf') {
          // open blockly editor
          atom.workspace.open('atom://open-diabetes-filter').then(editor => {
            // wait for blockly to be fully injected
            editor.blockly_promise.then(() => {
