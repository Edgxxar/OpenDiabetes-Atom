...
export default {
  ...
  loadBlockly() {
    ...
    Promise.all([
      ...
      if (atom.workspace.getCenter().getActivePaneItem() instanceof EditorView) {
  ...
  deserializeEditor(state) {
    ...
    return new EditorView(workspace);
  },

  deserializePlot(state) {
    return new PlotView(state.directory);
  },
