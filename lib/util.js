...
function getCli() {
  const native = atom.config.get('open-diabetes-filter.cli-native');
  if (native)
...
function checkCli(silent) {
  const command = atom.config.get('open-diabetes-filter.commands.version').replace('%cli%', getCli());
...
function getPlot() {
  const native = atom.config.get('open-diabetes-filter.plot-native');
  if (native)
...
function checkPlot(silent) {
  const command = atom.config.get('open-diabetes-filter.commands.plotversion').replace('%plot%', getPlot());
  cp.exec(command, (error, stdout, stderr) => {
    if (error !== null) {
...
module.exports = {
  getProjectPath, getModulePath, getRelativeProjectPath,
  getDocker, checkDocker,
  checkCli,
  getPlot, checkPlot,
  isVaultInitialized, initializeVault, warnNotInitialized,
