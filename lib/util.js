...
function spawnPlot(command, arguments = {}, callback = null, stdout = null, stderr = null) {
  const cmd = buildPlotCommand(command, arguments);
  console.debug(`Spawning command ${cmd}...`);
  let args = splitCommandArgs(cmd);
  const exec = args.shift();
  args = Object.freeze(args);
...
function buildPlotCommand(command, arguments) {
  arguments.plot = atom.config.get('open-diabetes-filter.plot');
  return buildCommand(command, arguments);
}
...
module.exports = {
  ...
  checkCli, isVaultInitialized,
  executeCli, spawnCli, spawnPlot,
  warnNotInitialized,
  checkPlot
};
