...
export default class EditorView {
  ...
  open(file) {
    if (this.toolbar.saved) {
      this.loadFromFile(file.path);
    } else {
      atom.confirm({
        message: 'Save unsaved changes from ' + (this.file ? this.file.getBaseName() : 'current workspace') + '?',
        type: 'question',
        buttons: ['Save', 'Discard']
      }, response => {
        if (response !== 0)
          return;
        this.save().then(() => this.loadFromFile(file.path));
      })
    }
  }
  ...
  loadFromFile(file) {
    ...
    this.file.read(true).then(content => {
      ...
      atom.notifications.addInfo('Workspace loaded from file', {
        description: 'Loaded `' + this.file.getBaseName() + '` from disk.'
      })
    });
  }
    ...
    file.write(this.output.code)
      .then(() => {
        ...
        if (compile) {
          ...
          }).then(() => Promise.resolve(new File(path.join(util.getProjectPath(), 'Process.class'))))
