...
export default class Toolbar {
  ...
  render() {
    return (
      <div className="odf-toolbar block">
        ...
        <div className="btn-group inline-block">
          <input className="input-file" id="odf-open-file" type="file" accept=".odf" on={{change: this.open}} title="Open..."/>
          <label htmlFor="odf-open-file" className="btn btn-info icon icon-file-directory"/>
          ...
          <button className="btn btn-info icon icon-chevron-down" title="Save workspace as..." on={{click: this.saveAs}}/>
        </div>
  ...
  open(event) {
    if (event.target.files.length > 0)
      this.editor.open(event.target.files[0]);
  }
