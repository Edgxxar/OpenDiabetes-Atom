...
export default class PlotView {
  ...
  render() {
    ...
    children.push(<Canvas ref="canvas1" scale={this.scale} url={this.canvas1}/>);
    ...
    if (this.labels && this.pdfs.length > 0 && this.slices.length > 0) {
      if (this.slices.length < this.currentPage) {
        ...
        const labels = this.slices
          ...
          .sort();
        children.push(<SliceLabel plot={this} labels={labels} currentSlice={this.slices[this.currentPage - 1]} tag={this.sliceTag}/>);
      }
    }
    ...
    csv.writeToPath(file, this.slices, {headers: true})
      .on('finish', () => {
        new Process('importslice', {
          file: file,
          tag: this.sliceTag
        }).execute().then(() => atom.notifications.addSuccess('Slices saved to disk.'));
      });
  }
    ...
    this.checkDataExists().then(exists => {
      ...
      new Promise((resolve, reject) => {
        this.checkSliceExists().then(exists => {
          ...
          const process = new Process('plot-' + this.view, args, PLOT);
          process.execute()
            .then(() => resolve())
            .catch(err => {
  ...
  watchFiles() {
    this.stopWatchFiles().then(() => {
      this.checkDirExists().then(exists => {
        if (!exists)
          return;
        this.updatePdfs(true);
  ...
  renderCurrentPdf() {
    ...
    switch (this.view) {
      case VIEW_DAILY:
      case VIEW_BIG:
        this.canvas1 = path.join(this.dir, this.pdfs[this.currentPage - 1]);
        break;
      case VIEW_NORMAL:
        this.canvas1 = path.join(this.dir, this.pdfs[this.currentPage - 1]);

        if (this.pdfs.length >= 2 && this.currentPage < this.pdfs.length) {
          this.canvas2 = path.join(this.dir, this.pdfs[this.currentPage]);
        } else {
          this.canvas2 = null;
        }
        break;
      case VIEW_TINY:
        ...
        if (this.pdfs.length >= 3) {
          if (this.currentPage < this.pdfs.length - 1) {
            this.canvas2 = path.join(this.dir, this.pdfs[this.currentPage]);
            this.canvas3 = path.join(this.dir, this.pdfs[this.currentPage + 1]);
