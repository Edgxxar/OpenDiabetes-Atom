...
export default class Toolbar {
    ...
    return (
      <div className="odf-toolbar block">
        ...
        <button className="btn btn-warning icon icon-checklist inline-block" title="Toggle Labels"
                on={{click: this.toggleLabel}} disabled={!this.hasSlice()}/>
        <div className="btn-group inline-block">
  ...
  hasSlice() {
    return this.view !== 'daily' && this.pdfs.length > 0;
  }

  toggleLabel() {
    this.plot.refs.slices.toggleLabel();
  }
  ...
  updateBtn() {
    ...
    this.plot.checkDataExists().then(exists => {
        ...
        switch (response) {
          ...
          case 2:
            ...
            this.allPdfs.reduce((promise, pdf) => {
              ...
              this.allPdfs = [];
              cache.unsetTagCache(this.getTag());
              this.plot.refs.slices.unloadSlices();
              resolve();
            });
            break;
        }
  ...
  watchFiles() {
    this.stopWatchFiles().then(() => {
      this.plot.checkDirExists().then(exists => {
        if (!exists)
          return;
        this.updatePdfs(true);
