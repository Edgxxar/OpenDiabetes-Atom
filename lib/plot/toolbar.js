...
export default class Toolbar {
  ...
  render() {
    ...
    if (this.pageInput) {
      ...
      page.push(<button className="btn btn-info" on={{click: this.openPageInput}}>{this.currentPage} / {this.maxPage}</button>);
    }
      ...
      <div className="odf-toolbar block">
        ...
        <div className="btn-group inline-block">
          ...
          <button className="btn btn-error icon icon-trashcan" on={{click: this.deleteBtn}}
                  disabled={this.inProgress} title="Delete all generated plots for this tag"/>
        </div>
        {progress}
      </div>
    );
