...
const {getCli, getPlot, getPlotConfig, getDocker, getProjectPath, splitCommandArgs} = require('./util');
...
class Process {
   ...
   */
  constructor(command, args = {}, type = CLI, docker = atom.config.get('open-diabetes-filter.docker.enabled')) {
  ...
  execute() {
    ...
    return new Promise((resolve, reject) => {
      if (this.docker) {
        ...
        this.process = new Promise(process_resolve => {
          const docker = getDocker();
          const stdout = typeof this.stdout === 'string' ? new BufferWritable(this.stdout) : new CallbackWritable(this.stdout);
          const stderr = typeof this.stderr === 'string' ? new BufferWritable(this.stderr) : new CallbackWritable(this.stderr);
          docker.run(this.docker_image, this.args, [stdout, stderr], {
            Tty: false,
            HostConfig: {
              Binds: this.binds,
            ...
            if (data && data.StatusCode === 0)
              resolve();
            else reject(data ? `Process returned with code ${data.StatusCode}` : err)
          }).on('container', container => {
            process_resolve(container);
          })
        });
        ...
        process.on('close', code => {
          if (code === 0)
            resolve();
          else reject(`Process returned with code ${code}`);
        });
        process.stdout.on('data', chunk => {
          typeof this.stdout === 'string' ? this.stdout += chunk.toString('utf8') : this.stdout(chunk.toString('utf8'));
        });
        process.stderr.on('data', chunk => {
          typeof this.stderr === 'string' ? this.stderr += chunk.toString('utf8') : this.stderr(chunk.toString('utf8'))
        });
